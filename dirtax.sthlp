...
{phang}
{opt rates} Numlist of tax rates which apply to every threshold specified in the tholds option. Note that the command, by default the command will sort the list. 

{phang}
{opt generate} Name of new variable which will contain the output vector of incomes.

{phang}
{opt tholds} Numlist with thresholds indicating where each tax bracket begins. Note that the command, by default the command will sort the list. 
